...
export default function SignupSigninModal() {
	...
	const handleSignup = async (e) => {
		...
		if (result.success && result.user) {
			toast.success(
				`Welcome, ${result.user.firstName}! Account created successfully.`
			);
			resetForm();
			closeModal();
		} else {
	...
	const handleSignin = async (e) => {
		...
		// First try as admin if email matches admin pattern
		if (email === "admin@example.com") {
			const adminResult = await signin(email, password, "Admin");
			if (adminResult.success) {
				toast.success(`Welcome Admin ${adminResult.user.firstName}!`);
				resetForm();
				closeModal();
				return;
			}
