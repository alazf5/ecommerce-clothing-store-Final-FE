...
export default function EditProduct() {
	...
	useEffect(() => {
		...
		try {
			...
			currentProducts = localProductsString
				? JSON.parse(localProductsString)
				: initialProductsData;
		} catch (e) {
			console.error(
				"Error parsing products from localStorage for edit page:",
				e
			);
		...
		productToEdit = currentProducts.find(
			(p) =>
	...
	const handleImageChange = (e) => {
		...
		if (file) {
			if (file.size > 1 * 1024 * 1024) {
				...
				setErrors((prev) => ({
					...prev,
					image: "File is too large (max 1MB).",
				}));
			...
			if (
				...
				setErrors((prev) => ({
					...prev,
					image: "Invalid file type (JPEG, PNG, GIF, WEBP).",
				}));
			...
			<div className="flex min-h-screen bg-gray-50">
				<main className="flex-1 p-6 sm:p-8 flex justify-center items-center">
					<p className="text-gray-600">
						Access Denied. Please sign in as a Seller.
					</p>
				</main>
			</div>
		);
		...
		<div className="flex min-h-screen bg-gray-50 font-sans">
			...
			<main className="flex-1 p-6 sm:p-8">
				<header className="mb-8">
					...
					<p className="text-sm text-gray-500 mt-1">
						...
						<span className="font-medium text-gray-700">
							{formData.name || originalProductName || "Product"}
						</span>
					</p>
				</header>
				...
				<form
					...
					className="bg-white p-6 sm:p-8 rounded-xl shadow-xl space-y-6 max-w-3xl mx-auto"
				>
					{/* Product Name */}
					<div>
						<label
						...
						<input
							...
							onBlur={handleBlur}
							className={`w-full px-4 py-2.5 border rounded-lg shadow-sm focus:outline-none focus:ring-2 sm:text-sm ${
								errors.name
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div>
							...
							<select
								...
								onChange={handleChange}
								onBlur={handleBlur}
								className={`w-full px-4 py-2.5 border rounded-lg shadow-sm focus:outline-none focus:ring-2 sm:text-sm bg-white ${
									errors.category
										? "border-red-500 focus:ring-red-500"
										: "border-gray-300 focus:ring-blue-500 focus:border-blue-500"
								}`}
					...
					<div>
						...
						{errors.image && (
							<p className="text-xs text-red-600 mt-1.5">{errors.image}</p>
						)}
						{/* Informative text if showing an existing image and no new file is selected */}
						{imagePreview &&
							!selectedFile &&
							(formData.name ||
								originalProductName) /* Check if it's an existing product being edited */ && (
								<p className="text-xs text-gray-500 mt-1 text-center">
									Currently showing existing image. Upload a new file to change
									it, or click 'X' to remove.
								</p>
							)}
					</div>
