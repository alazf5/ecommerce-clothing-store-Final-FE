...
import {
	...
	sendMockMessage,
	getMockUserById,
} from "../../data/mockMessages"; // Using mock data
	...
	return (
		...
		>
			<div className="flex-shrink-0 h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center text-gray-500 text-sm">
				{otherParticipant.avatarFallback ||
			...
			<div className="flex-1 min-w-0">
				<div className="flex justify-between items-center">
					...
					>
						{lastMessageTimestamp
							? formatDistanceToNowStrict(parseISO(lastMessageTimestamp), {
									addSuffix: true,
							  })
							: ""}
					</p>
		...
		>
			...
			>
				{!isCurrentUserSender && ( // Show name only if it's the other person (Buyer)
					<p className="text-xs font-semibold mb-0.5 text-gray-600">
						{senderName}
					</p>
				)}
...
export default function SellerMessages() {
	...
	const handleSelectConversation = useCallback(
		(conversationId) => {
			...
			setConversations((prev) =>
				prev.map((c) =>
					c.id === conversationId
						? { ...c, isUnread: false, unreadMessagesCount: 0 }
						: c
				)
			);
		},
	...
	const handleSendMessage = async (e) => {
		...
		setIsSending(true);
		setTimeout(() => {
			const updatedConversation = sendMockMessage(
				selectedConversation.id,
				currentUser.id,
				newMessage.trim()
			);
			if (updatedConversation) {
				const refreshedConvo = getConversationById(
					selectedConversation.id,
					currentUser.id
				);
		...
		return (
			<div className="flex h-screen bg-gray-100">
				<Sidebar
					links={sellerLinks}
					userRole="SELLER"
					userName={currentUserDetails?.name || "User"}
				/>
			...
			<div className="flex h-screen bg-gray-100">
				...
				<main className="flex-1 p-6 flex flex-col justify-center items-center text-center">
					...
					<p className="text-gray-600">
						Please sign in as a Seller to view your messages.
					</p>
				</main>
			</div>
		);
					...
					{selectedConversation ? (
						<>
							<div className="p-3 sm:p-4 border-b border-gray-200 bg-white flex items-center space-x-3 shadow-sm">
								...
								<div className="flex-shrink-0 h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center text-gray-500 text-sm">
									{selectedConversation.otherParticipant?.avatarFallback ||
							...
							<div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
								{selectedConversation.messages.map((msg) => (
									<ChatMessageBubble
										...
										senderDetails={
											selectedConversation.participantsDetails[msg.senderId]
										}
									/>
								))}
								{isSending && (
									<p className="text-xs text-gray-400 italic text-center my-2">
										Sending...
									</p>
								)}
							</div>
