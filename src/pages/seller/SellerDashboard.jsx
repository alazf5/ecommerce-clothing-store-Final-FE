...
export default function SellerDashboard() {
	...
	useEffect(() => {
		...
		if (currentUser && userRole === "SELLER") {
			const sellerProducts = productsData.filter(
				(p) => String(p.sellerId) === String(currentUser.id)
			);
			setProductCount(sellerProducts.length);

			const allMessages = JSON.parse(localStorage.getItem("messages")) || [];
			const sellerReceivedMessages = allMessages.filter(
				(msg) => String(msg.receiverId) === String(currentUser.id)
			);
			setMessageCount(sellerReceivedMessages.length);

			setTotalSales(
				sellerProducts.reduce(
					(acc, curr) => acc + curr.price * (Math.floor(Math.random() * 5) + 1),
					0
				)
			);
		}
	...
	const recentActivity = [
		{
			id: 1,
			text: `You listed "Vintage Leather Jacket".`,
			time: "2 hours ago",
			type: "product",
		...
		{
			id: 2,
			text: `New message from BuyerX regarding "Handmade Scarf".`,
			time: "5 hours ago",
			type: "message",
			linkTo: "/seller/messages",
		},
		{
			id: 3,
			text: `Your product "Classic Blue Jeans" received 10 views today.`,
			time: "1 day ago",
			type: "stats",
			linkTo: "#",
		},
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					<div className="bg-white p-6 rounded-xl shadow-lg">
						...
						<ul className="space-y-3">
							{sellerLinks
								.filter((link) => link.path !== "/seller/dashboard")
								.map((link) => (
									<li key={link.path}>
										<Link
											to={link.path}
											className="flex items-center gap-3 p-3 rounded-lg text-gray-600 hover:bg-gray-100 hover:text-blue-600 transition-colors group"
										>
											{link.icon && (
