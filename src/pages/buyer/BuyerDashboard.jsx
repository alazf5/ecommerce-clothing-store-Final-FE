...
export default function BuyerDashboard() {
	...
	useEffect(() => {
		...
		if (currentUser && userRole === "BUYER") {
			...
			const buyerSentMessages = allMessages.filter(
				(msg) => String(msg.senderId) === String(currentUser.id)
			);
	...
	if (!currentUser || userRole !== "BUYER") {
		return (
			<div className="flex justify-center items-center min-h-screen bg-gray-100">
				<p>
					Access denied or user data not available. Please ensure you are signed
					in as a Buyer.
				</p>
			</div>
		);
			...
			<main className="flex-1 p-6 sm:p-8 space-y-8">
				<div>
					...
					<p className="text-sm text-gray-600 mt-1">
						Welcome to your personal dashboard. Manage your orders, messages,
						and more.
					</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					<Link
						to="/buyer/orders"
						className="block bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1"
					>
						...
						<p className="text-xs text-blue-600 mt-4 hover:underline">
					...
					>
						...
						<p className="text-xs text-purple-600 mt-4 hover:underline">
							View messages &rarr;
						</p>
					</Link>
					...
					>
						...
						<p className="text-xs text-green-600 mt-4 hover:underline">
							Edit profile &rarr;
						</p>
					</Link>
				</div>
					...
					<div className="bg-white p-6 rounded-xl shadow-lg">
						...
						<ul className="space-y-4">
							{recentOrders.map((order) => (
								<li
									key={order.id}
									className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow bg-gray-50/50"
								>
									<div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
										<div className="flex-grow min-w-0">
											...
											<p className="text-xs text-gray-500">
												Order ID: {order.id.substring(0, 15)}...
											</p>
										</div>
				...
				{recommendedProducts.length === 0 && recentOrders.length === 0 && (
					<div className="bg-white p-10 rounded-xl shadow-lg text-center mt-8">
						...
						<p className="text-gray-500 mb-6">
							Start shopping or place an order to see activity here.
						</p>
						<Link
							to="/products"
